Rails.application.routes.draw do
  # Define your application routes per the DSL in https://guides.rubyonrails.org/routing.html

  # Reveal health status on /up that returns 200 if the app boots with no exceptions, otherwise 500.
  # Can be used by load balancers and uptime monitors to verify that the app is live.
  get "up" => "rails/health#show", as: :rails_health_check
  resources :customers do 
    post 'add_company', on: :member
    post 'pay', on: :member
    post 'withdraw', on: :member
  end 
  resources :memberships, only: [:create, :destroy]
  resources :payments, only: [:index, :create, :show] 

  get 'payments/customers/:customer_id', to: 'payments#customer_payments', as: 'customer_payments'
  get 'payments/companies/:company_id', to: 'payments#company_payments', as: 'company_payments'
  get 'companies/customers/:customer_id', to: 'companies#customer_companies', as: 'customer_companies'

  resources :withdrawals, only: [:index, :create, :show]
  resources  :companies do 
    get 'balance', on: :member
  end  


  # Defines the root path route ("/")
  # root "posts#index"
end
